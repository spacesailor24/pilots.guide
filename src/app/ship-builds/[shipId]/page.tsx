...
export default function ShipBuildPage({
  ...
  useEffect(() => {
    async function fetchShipData() {
      try {
        const response = await fetch(`/api/ships/${resolvedParams.shipId}`);
        if (!response.ok) {
        ...
        // Set initial selected build (featured or first)
        const featuredBuild = shipData.builds.find((build: Build) => build.featured);
        setSelectedBuild(featuredBuild || shipData.builds[0]);
      } catch (error) {
